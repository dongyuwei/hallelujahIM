...
> 注意：p50 是 9 个前缀**混合**后的统计值，会被命中数少的前缀拉低——例如 SQLite range
> 的 0.026 ms 只代表 `tes` 这类输入。最差的一个字前缀 `a`（9,242 项）在下面单独列出，
> 它的耗时由 `ORDER BY frequency` 排序主导，而不是索引扫描（这也是它只比 `LIKE` 快约
> 2.7 倍的原因）。

| 实现 | SQLite `LIKE`（基线） | SQLite 索引范围 | LMDB 前缀 | SQLite 精确 | LMDB 精确 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
- `LIKE` 查询（编写基准测试时应用的实现）在三种实现中的 p50 都约为 5.4–5.6 ms，是最慢的前缀查询方案；应用改用索引范围查询后不再全表扫描。
