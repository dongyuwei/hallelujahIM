...
9. **拼音输入中文(Pinyin to Chinese)**：按`右Command` 键切换到拼音输入模式，输入拼音即可打出中文汉字，`空格` 或数字键提交高亮候选，`Enter` 回车键提交高亮候选（不附加空格）。再次按`右Command` 切回智能英语输入模式。开启「中英混合输入」偏好后无需切换模式，输入时同时给出英文与中文候选：每块最多 5 个候选，页面剩余位置由另一语言补满，并且会根据上一次提交的内容动态排序（上次输入中文或中文标点则中文候选在上，反之英文或英文标点在上）。
